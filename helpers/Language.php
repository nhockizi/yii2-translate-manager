...
namespace nhockizi\translatemanager\helpers;

use Yii;
use nhockizi\translatemanager\services\Scanner;
use nhockizi\translatemanager\bundles\TranslationPluginAsset;
...
class Language
{
     ...
     *      ],
     * ];
     * $result = \nhockizi\translatemanager\helpers\Language::a($array, $params);
     * ~~~
     * 
     * The result:
     * 
    ...
    {
        return Yii::$app->session->has(\nhockizi\translatemanager\Module::SESSION_KEY_ENABLE_TRANSLATE);
    }
    ...
    {
        $languageSources = \nhockizi\translatemanager\models\LanguageSource::find()->where(['category' => $category])->all();
        ...
        if (empty($messages[$message])) {
            $languageSource = new \nhockizi\translatemanager\models\LanguageSource;
    ...
    {
        $languageSources = \nhockizi\translatemanager\models\LanguageSource::find()->select('category')->distinct()->all();
